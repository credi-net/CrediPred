...
class WebGraphDataset(InMemoryDataset, ABC):
    """Graph dataset with preprocessing and target generation.
    ...
    def __init__(
        ...
        embedding_lookup: Optional[str] = None,
    ):
        ...
        if embedding_location:
            self.embedding_location = pathlib.Path(embedding_location)
        else:
    ...
    def process(self) -> None:
        ...
        if not os.path.exists(target_path):
            self._ensure_target_file(node_path=node_path, target_path=target_path)
        ...
        x_full, mapping, full_index = load_node_csv(
            ...
            embedding_lookup=str(self.embedding_lookup),
            index_col=0,
        ...
        score, idx_dict = self._process_targets_and_splits(
            target_path=target_path, mapping=mapping, full_index=full_index
        )
        ...
        for split, idx in idx_dict.items():
        ...
        self.verify_stratification(data=data, dict_split=idx_dict)

        torch.save(mapping, self.processed_dir + '/mapping.pt')
        torch.save(self.collate([data]), self.processed_paths[0])

    @abstractmethod
    def _ensure_target_file(self, node_path: str, target_path: str) -> None:
        """Hook for target generation logic."""

    @abstractmethod
    def _process_targets_and_splits(
        self, target_path: str, mapping: dict, full_index: pd.Index
    ) -> Tuple[torch.Tensor, Dict]:
        """Hook for the core logic that differs between subclasses."""
...
class WebGraphDatasetRegression(WebGraphDataset):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            target_index_col=target_index_col,
...
class WebGraphDatasetBinaryDownsample(WebGraphDataset):
    ...
    def _process_targets_and_splits(
        ...
        logging.info(f'Pre-downsampled size valid: {len(valid_idx_list)}')
        ...
        valid_idx, valid_removed = downsample_to_balanced(valid_idx_list, score)
        logging.info(f'Valid size: {valid_idx.size()}')
        test_idx, test_removed = downsample_to_balanced(test_idx_list, score)
        logging.info(f'Test size: {test_idx.size()}')
