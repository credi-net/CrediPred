...
from credipred.utils.target_generation import (
    ...
    generate_multihead_targets_csv,
    reverse_domain,
)
...
class TemporalBinaryDatasetGlobalSplits(InMemoryDataset):
    """Graph dataset with temporal / versioned preprocessing and binary target generation.
...
class TemporalDatasetGlobalSplit(InMemoryDataset):
    """Graph dataset with temporal / versioned preprocessing and target generation.
...
class TemporalBinaryDatasetAllGlobalSplits(InMemoryDataset):
    """Graph dataset with temporal / versioned preprocessing and binary target generation.
...
class TemporalBinaryDatasetAllMultiGlobalSplits(InMemoryDataset):
    """Graph dataset with temporal / versioned preprocessing and binary target generation.
    ...
    def __init__(
        ...
        edge_file: str = 'edges.csv',
        target_file: str = 'target.csv',
        target_col: Union[List[str], str] = 'weak_label',
        target_index_name: str = 'nid',
        target_index_col: int = 0,
        edge_src_col: str = 'src',
        edge_dst_col: str = 'dst',
    ...
    def process(self) -> None:
        ...
        if os.path.exists(target_path):
            ...
            logging.info('Generating binary target file.')
            label_dict = get_multihead_dict()
            generate_multihead_targets_csv(node_path, target_path, label_dict)
        ...
        df_target = pd.read_csv(target_path)
        logging.info(f'Size of target dataframe: {df_target.shape}')
        logging.info(f'Sample of target dataframe: {df_target.head()}')
